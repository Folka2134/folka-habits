  ...
  const handleLogSession = (subjectId: string, minutes: { input: number; output: number }) => {
    setSubjects((prevSubjects) => {
      return prevSubjects.map((subject) => {
        ...
        const totalRequired = levelConfig.inputMinutes + levelConfig.outputMinutes
        ...
        const alreadyLoggedToday = subject.sessions.some((session) => session.date === today)
        ...
        if (meetsRequirement) {
          ...
          if (!alreadyLoggedToday) {
            ...
            const lastSessionDate =
              subject.sessions.length > 0 ? subject.sessions[subject.sessions.length - 1].date : null

            if (lastSessionDate === yesterdayStr || subject.sessions.length === 0) {
  ...
  if (subjects.length === 0) {
    return (
      <div className="text-center p-8 border border-dashed rounded-lg">
        ...
        <p className="text-muted-foreground mb-4">Add your first subject to start tracking your study progress</p>
      </div>
    )
  }
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {subjects.map((subject) => {
        ...
        // Check if already logged today
        const today = new Date().toISOString().split("T")[0]
        const loggedToday = subject.sessions.some((session) => session.date === today && session.meetsRequirement)
            ...
            <CardContent className="pb-2">
              <div className="flex justify-between mb-1 text-sm">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>{levelConfig.inputMinutes + levelConfig.outputMinutes} min/day</span>
                </div>
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                <div className="border rounded p-2">
                  ...
                  <div className="font-medium">{levelConfig.inputMinutes} min</div>
                </div>
                <div className="border rounded p-2">
                  <div className="text-muted-foreground">Output</div>
                  <div className="font-medium">{levelConfig.outputMinutes} min</div>
                </div>
              </div>
            </CardContent>
